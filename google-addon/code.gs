    ...
    if (text.length > MAX_TEXT_LENGTH) {
      return createErrorCard(
        "Document is too long (" +
          ...
          MAX_TEXT_LENGTH +
          " characters. Try analyzing a selection instead.",
      );
    }
...
function callBiasAPI(text) {
  ...
  if (API_URL === "https://your-api-url.com/analyze") {
    throw new Error(
      "API URL not configured. Please update API_URL in the script.",
    );
  }
  ...
  try {
    ...
    if (responseCode !== 200) {
      ...
      throw new Error(
        "API request failed (status " + responseCode + "). Please try again.",
      );
    }
    ...
    if (e.message.includes("Address unavailable")) {
      throw new Error(
        "Could not connect to the bias detection service. Please check your internet connection.",
      );
...
function createResultCard(issues, source) {
  ...
  if (issues.length === 0) {
    headerSection.addWidget(
      CardService.newTextParagraph().setText(
        '<font color="#059669"><b>No bias detected</b></font><br><font color="#6b7280">Your text uses inclusive language.</font>',
      ),
    );
...
function createErrorCard(message) {
  ...
  section.addWidget(
    CardService.newTextParagraph().setText(
      '<font color="#dc2626"><b>Error</b></font>',
    ),
  );

  section.addWidget(
    CardService.newTextParagraph().setText(
      '<font color="#374151">' + message + "</font>",
    ),
  );
...
function findInDocument(e) {
  ...
  while (searchFrom) {
    ...
    if (Math.abs(foundStart - startPos) < 5) {
  ...
  return CardService.newActionResponseBuilder().setStateChanged(false).build();
}
